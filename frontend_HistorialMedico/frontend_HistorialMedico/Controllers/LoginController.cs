    ...
    public class LoginController : Controller
    {
        ...
        public async Task<JsonResult> Loginusuario(string wusuario, string wclave)
        {
            try
            {
                ...
                {
                    client.BaseAddress = new Uri(MvcApplication.wsRouteBackend);
                ...
                DVariables["USUARIO"] = oEnUsuario.Susuario.ToString();
        ...
        [HttpPost]
        public async Task<JsonResult> Crearusuario(string wnombres, string wamaterno, string wapaterno, int wtipousuario,
                                              string wusuario, string wclave)
        {
            try
            {
                var objResultado = new object();
                ...
                using (var client = new HttpClient())
                {
                    ...
                    client.DefaultRequestHeaders.Clear();
                    client.DefaultRequestHeaders.Accept.Add(new MediaTypeWithQualityHeaderValue("application/json"));
                    HttpResponseMessage ResRegistrarCuenta = await client.GetAsync("api/usuario/wsInsertarCuenta?wnombres=" + wnombres
                        + "&wamaterno=" + wamaterno + "&wapaterno=" + wapaterno + "&wgenero=" + 1 +
                        "&wcorreo=" + "");
                ...
                using (var client = new HttpClient())
                {
                    ...
                    HttpResponseMessage ResCrearCuenta = await client.GetAsync("api/usuario/wsInsertarUsuario?widusuario=" + idusuarioGenerado
                        + "&witipousuario=" + wtipousuario + "&wusuario=" + wusuario + "&wclave=" + wclave);
