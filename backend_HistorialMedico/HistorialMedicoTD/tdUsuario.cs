...
namespace HistorialMedicoTD
{
    public class tdUsuario : td_aglobal
    {
        ...
        public int tdInsertarCuenta(string tdnombres, string tdamaterno, string tdapaterno
                                    , string tdgenero, string tdcorreo)
        {
            int iRespuesta = -3;
            try
            {
                using (MySqlConnection con = new MySqlConnection(mysqlConexion))
                {
                    ...
                    using (MySqlTransaction scope = con.BeginTransaction())
                    {
                        ...
                        iRespuesta = iadUsuario.adInsertarCuenta(tdnombres, tdamaterno, tdapaterno
                                                                , tdgenero, tdcorreo);
                        scope.Commit();
                    }
                }
        ...
        public int tdInsertarUsuario(int tdidusuario, int tditipousuario, string tdusuario, string tdclave)
        {
            int iRespuesta = -1;
            try
            {
                using (MySqlConnection con = new MySqlConnection(mysqlConexion))
                {
                    ...
                    {
                        iadUsuario = new adUsuario(con);
                        iRespuesta = iadUsuario.adInsertarUsuario(tdidusuario, tditipousuario, tdusuario, tdclave);
                        scope.Commit();
                    }
                }
                return (iRespuesta);
